import React, { useState, useEffect } from 'react';
import { ProjectClassificationType, ContactFormInput } from '../types';
import { DIRECT_LINES, CONTACT_EMAIL } from '../data';
import { Phone, Mail, Instagram, Cpu, Send, Loader2, CheckCircle2, History, Database, ShieldAlert, Trash2, MessageCircle, Headphones } from 'lucide-react';
import instagramQr from '../assets/images/instagram_qr_1781853591108.jpg';

interface ContactViewProps {
  selectedClassification: ProjectClassificationType;
  setSelectedClassification: (val: ProjectClassificationType) => void;
}

interface SubmittedLog extends ContactFormInput {
  id: string;
  timestamp: string;
  status: 'QUEUED' | 'TRANSMITTED' | 'PROCESSED';
}

export default function ContactView({ selectedClassification, setSelectedClassification }: ContactViewProps) {
  // Input fields state
  const [name, setName] = useState('');
  const [phone, setPhone] = useState('');
  const [email, setEmail] = useState('');
  const [message, setMessage] = useState('');
  
  // Submit state
  const [submitState, setSubmitState] = useState<'idle' | 'processing' | 'success'>('idle');
  const [historyLogs, setHistoryLogs] = useState<SubmittedLog[]>([]);

  // Load from local storage
  useEffect(() => {
    try {
      const saved = localStorage.getItem('nexgen_transmissions');
      if (saved) {
        setHistoryLogs(JSON.parse(saved));
      }
    } catch (e) {
      console.error(e);
    }
  }, []);

  const handleClassificationChange = (type: ProjectClassificationType) => {
    setSelectedClassification(type);
  };

  const getWhatsappUrl = () => {
    const base = 'https://wa.me/918100297169';
    if (!name && !email && !message && !phone) {
      return base;
    }
    const messageLines = [
      `Hello NexGen WebWorks,`,
      `Name: ${name || 'N/A'}`,
      `Phone: ${phone || 'N/A'}`,
      `Email: ${email || 'N/A'}`,
      `Project Category: ${getClassificationLabel(selectedClassification)}`,
      `Details: ${message || 'Standard Consult Request'}`
    ];
    return `${base}?text=${encodeURIComponent(messageLines.join('\n'))}`;
  };

  const getWhatsappUrlForMain = () => {
    const base = 'https://wa.me/917003091321';
    if (!name && !email && !message && !phone) {
      return base;
    }
    const messageLines = [
      `Hello NexGen WebWorks,`,
      `Name: ${name || 'N/A'}`,
      `Phone: ${phone || 'N/A'}`,
      `Email: ${email || 'N/A'}`,
      `Project Category: ${getClassificationLabel(selectedClassification)}`,
      `Details: ${message || 'Standard Consult Request'}`
    ];
    return `${base}?text=${encodeURIComponent(messageLines.join('\n'))}`;
  };

  const handleDirectAction = (actionType: 'whatsapp' | 'direct_call' | 'support_call', e?: React.MouseEvent) => {
    if (name || email || message || phone) {
      const actionName = actionType === 'whatsapp' ? 'WhatsApp Chat' : actionType === 'direct_call' ? 'Direct phone call' : 'Support phone call';
      const newLog: SubmittedLog = {
        id: 'TX-' + Math.floor(100000 + Math.random() * 900000),
        name: name || 'Anonymous',
        email: email || 'Direct-action@nexgen.in',
        phone: phone || undefined,
        classification: selectedClassification,
        message: `Connected via ${actionName}. Message specs: ${message || 'N/A'}`,
        timestamp: new Date().toLocaleTimeString(),
        status: 'TRANSMITTED',
      };
      const updated = [newLog, ...historyLogs];
      setHistoryLogs(updated);
      localStorage.setItem('nexgen_transmissions', JSON.stringify(updated));
    }
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    handleDirectAction('whatsapp');
    window.open(getWhatsappUrlForMain(), '_blank', 'noopener,noreferrer');
  };

  const clearHistory = () => {
    if (confirm('Clear local transmission log history?')) {
      setHistoryLogs([]);
      localStorage.removeItem('nexgen_transmissions');
    }
  };

  // Human readable labels for tags
  const getClassificationLabel = (type: ProjectClassificationType) => {
    switch (type) {
      case 'web_app':
        return 'Web Application';
      case 'corporate_site':
        return 'Corporate Platform';
      case 'ecommerce':
        return 'E-Commerce Architecture';
      case 'other_protocol':
        return 'Other Protocol';
    }
  };

  return (
    <div className="flex flex-col w-full px-6 md:px-12 max-w-7xl mx-auto py-10" id="contact-canvas">
      
      {/* TWO COLUMN GRID LAYOUT */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start text-left w-full">
        
        {/* LEFT COLUMN: CONTACT CARDS BENTO */}
        <div className="lg:col-span-5 flex flex-col gap-10">
          <div>
            <h1 className="font-display text-4xl sm:text-5xl md:text-5xl font-extrabold text-white mb-4 leading-tight">
              Let's Build The Future.
            </h1>
            <p className="font-sans text-body-lg text-on-surface-variant font-light leading-relaxed">
              Ready to elevate your digital presence? Our engineering team is standing by to discuss your next high-performance web project.
            </p>
          </div>

          {/* Contact Details Bento Cards Grid */}
          <div className="flex flex-col gap-4">
            
            {/* Phone Card */}
            <div className="glass-panel p-6 rounded-2xl flex items-start gap-4 hover:bg-white/5 transition-colors cursor-default border-white/5 shadow-md">
              <div className="p-3.5 rounded-lg bg-secondary-container/30 text-tertiary flex items-center justify-center border border-[#8A2BE2]/20">
                <Phone className="w-5 h-5 text-tertiary" />
              </div>
              <div className="flex flex-col text-left">
                <h3 className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1 font-semibold">
                  Phone
                </h3>
                {DIRECT_LINES.map((line) => (
                  <p key={line.number} className="font-sans text-body-md text-white font-medium mt-0.5">
                    <span>{line.number}</span>
                    {line.label && (
                      <span className="text-on-surface-variant text-xs font-light ml-1.5">
                        {line.label}
                      </span>
                    )}
                  </p>
                ))}
              </div>
            </div>

            {/* Email Card */}
            <a 
              href={`mailto:${CONTACT_EMAIL}`}
              className="glass-panel p-6 rounded-2xl flex items-start gap-4 hover:bg-white/5 transition-all text-left border-white/5 group shadow-md"
            >
              <div className="p-3.5 rounded-lg bg-secondary-container/30 text-tertiary flex items-center justify-center border border-[#8A2BE2]/20 group-hover:bg-[#8A2BE2]/20 transition-colors">
                <Mail className="w-5 h-5 text-tertiary" />
              </div>
              <div className="flex flex-col">
                <h3 className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1 font-semibold">
                  Email
                </h3>
                <span className="font-sans text-body-md text-white font-medium group-hover:text-tertiary transition-colors mt-0.5">
                  {CONTACT_EMAIL}
                </span>
              </div>
            </a>

            {/* Instagram Card with QR Code */}
            <a 
              href="https://instagram.com/nexgen_webworks_in"
              target="_blank"
              rel="noopener noreferrer"
              className="glass-panel p-6 rounded-2xl flex flex-col gap-5 hover:bg-white/5 transition-all text-left border-white/5 group shadow-md"
            >
              <div className="flex items-start gap-4">
                <div className="p-3.5 rounded-lg bg-secondary-container/30 text-tertiary flex items-center justify-center border border-[#8A2BE2]/20 group-hover:bg-[#8A2BE2]/20 transition-colors">
                  <Instagram className="w-5 h-5 text-tertiary animate-pulse" />
                </div>
                <div className="flex flex-col text-left">
                  <h3 className="font-sans text-label-sm text-on-surface-variant uppercase tracking-wider mb-1 font-semibold">
                    Instagram
                  </h3>
                  <span className="font-sans text-body-md text-white font-medium group-hover:text-tertiary transition-colors mt-0.5">
                    @nexgen_webworks_in
                  </span>
                </div>
              </div>

              {/* Centered QR scan element */}
              <div className="relative mt-1 p-5 bg-[#07070a]/90 border border-outline-variant/30 rounded-xl overflow-hidden flex flex-col items-center justify-center group-hover:border-tertiary/40 transition-colors">
                <div className="absolute top-2 right-2.5 flex items-center gap-1 bg-black/60 px-1.5 py-0.5 rounded border border-white/5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-[7px] font-mono text-zinc-400 font-bold uppercase tracking-wider">LCP: Clean</span>
                </div>

                <div className="w-64 h-64 relative mt-2 bg-white rounded-xl p-2 border border-white/10 shadow-lg group-hover:scale-105 transition-transform duration-300">
                  <img 
                    src={instagramQr} 
                    alt="Instagram Official QR Protocol for @nexgen_webworks_in" 
                    className="w-full h-full object-contain rounded-lg"
                    referrerPolicy="no-referrer"
                  />
                </div>

                <span className="text-[10px] font-mono text-on-surface-variant tracking-wider uppercase mt-4 text-center text-zinc-400 font-light">
                  SCAN RADAR WITH INSTAGRAM LENS
                </span>
              </div>
            </a>

          </div>
        </div>

        {/* RIGHT COLUMN: INTERACTIVE CONNECTION FORM */}
        <div className="lg:col-span-7 w-full">
          <div className="glass-panel p-8 md:p-11 rounded-2xl relative overflow-hidden border-white/5 shadow-2xl">
            {/* Ambient internal soft purple glow */}
            <div className="absolute top-0 right-0 w-64 h-64 bg-tertiary/10 blur-[80px] rounded-full pointer-events-none select-none" />

            <h2 className="font-display text-3xl font-bold text-white mb-8 tracking-tight">
              Contact Us
            </h2>

            <form onSubmit={handleSubmit} className="flex flex-col gap-6 relative z-10">
              
              {/* Inputs row */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Full name */}
                <div className="flex flex-col gap-2">
                  <label className="font-sans text-label-sm text-on-surface-variant font-medium text-left" htmlFor="name">
                    Full Name
                  </label>
                  <input 
                    type="text" 
                    id="name"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Jane Doe"
                    required
                    className="bg-[#050505] border border-outline-variant/60 rounded-lg p-3.5 text-white font-sans focus:outline-none focus:border-tertiary transition-all duration-300 placeholder:text-zinc-600 shadow-inner"
                  />
                </div>

                {/* Email Address */}
                <div className="flex flex-col gap-2">
                  <label className="font-sans text-label-sm text-on-surface-variant font-medium text-left" htmlFor="email">
                    Email Address
                  </label>
                  <input 
                    type="email" 
                    id="email"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="jane@company.com"
                    required
                    className="bg-[#050505] border border-outline-variant/60 rounded-lg p-3.5 text-white font-sans focus:outline-none focus:border-tertiary transition-all duration-300 placeholder:text-zinc-600 shadow-inner"
                  />
                </div>

                {/* Phone Number Block */}
                <div className="flex flex-col gap-2">
                  <label className="font-sans text-label-sm text-on-surface-variant font-medium text-left" htmlFor="phone">
                    Phone Number
                  </label>
                  <input 
                    type="tel" 
                    id="phone"
                    value={phone}
                    onChange={(e) => setPhone(e.target.value)}
                    placeholder="+91 XXXXX XXXXX"
                    className="bg-[#050505] border border-outline-variant/60 rounded-lg p-3.5 text-white font-sans focus:outline-none focus:border-tertiary transition-all duration-300 placeholder:text-zinc-600 shadow-inner"
                  />
                </div>
              </div>

              {/* Project Classification Radio pills selection */}
              <div className="flex flex-col gap-2">
                <label className="font-sans text-label-sm text-on-surface-variant font-medium mb-2">
                  Project Classification
                </label>
                <div className="flex flex-wrap gap-3">
                  {[
                    { label: 'Web Application', val: 'web_app' as ProjectClassificationType },
                    { label: 'Corporate Platform', val: 'corporate_site' as ProjectClassificationType },
                    { label: 'E-Commerce Architecture', val: 'ecommerce' as ProjectClassificationType },
                    { label: 'Other Protocol', val: 'other_protocol' as ProjectClassificationType },
                  ].map((chip) => {
                    const isSelected = selectedClassification === chip.val;
                    const pillStyle = isSelected 
                      ? 'bg-secondary-container border-tertiary text-white shadow-lg shadow-secondary-container/30 glow-accent font-semibold' 
                      : 'bg-surface-container-low border-outline-variant/40 text-on-surface hover:border-white/20 hover:bg-white/5';

                    return (
                      <button
                        key={chip.val}
                        type="button"
                        onClick={() => handleClassificationChange(chip.val)}
                        className={`px-4 py-2 text-xs font-sans rounded-full border transition-all duration-300 cursor-pointer ${pillStyle}`}
                        id={`classification-chip-${chip.val}`}
                      >
                        {chip.label}
                      </button>
                    );
                  })}
                </div>
              </div>

              {/* Message specs */}
              <div className="flex flex-col gap-2">
                <label className="font-sans text-label-sm text-on-surface-variant font-medium text-left" htmlFor="message">
                  Description of your requirement
                </label>
                <textarea 
                  id="message"
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  placeholder="Detail your project requirements, technical constraints, and objectives here..."
                  rows={5}
                  className="bg-[#050505] border border-outline-variant/60 rounded-lg p-3.5 text-white font-sans focus:outline-none focus:border-tertiary transition-all duration-300 placeholder:text-zinc-600 resize-none shadow-inner"
                />
              </div>

              {/* Big Send to WhatsApp Button of Number +91 7003091321 */}
              <div className="pt-2">
                <a 
                  href={getWhatsappUrlForMain()}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => handleDirectAction('whatsapp')}
                  className="flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-500 text-white w-full py-4 rounded-xl font-sans text-sm font-bold scale-100 hover:scale-[1.01] active:scale-[0.99] transition-all duration-300 text-center shadow-[0_8px_25px_rgba(16,185,129,0.3)] border border-[#10b981]/50 group"
                >
                  <MessageCircle className="w-5 h-5 text-white animate-pulse" />
                  <span>Send to WhatsApp</span>
                </a>
              </div>

              {/* Communication Protocols Matrix */}
              <div className="pt-6 border-t border-outline-variant/30 mt-4 flex flex-col gap-4">
                <div className="text-left flex items-center justify-between">
                  <span className="font-sans text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                    Select Communication Protocol
                  </span>
                  <span className="hidden sm:inline-block text-[8px] font-mono text-zinc-600">
                    SECURE REDIRECT ENCRYPTION
                  </span>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {/* WhatsApp Direct */}
                  <a 
                    href={getWhatsappUrl()}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={(e) => handleDirectAction('whatsapp', e)}
                    className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-3.5 rounded-xl font-sans text-xs font-bold hover:scale-[1.02] active:scale-[0.98] cursor-pointer border border-[#10b981]/45 transition-all duration-200 text-center shadow-[0_4px_15px_rgba(16,185,129,0.2)]"
                  >
                    <MessageCircle className="w-4.5 h-4.5 text-white animate-pulse" />
                    <span>WhatsApp Chat</span>
                  </a>

                  {/* Direct Call */}
                  <a 
                    href="tel:+917003091321"
                    onClick={(e) => handleDirectAction('direct_call', e)}
                    className="flex items-center justify-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white px-4 py-3.5 rounded-xl font-sans text-xs font-bold hover:scale-[1.02] active:scale-[0.98] cursor-pointer border border-white/10 transition-all duration-200 text-center shadow-lg"
                  >
                    <Phone className="w-4.5 h-4.5 text-white" />
                    <span>Direct Call</span>
                  </a>

                  {/* Call Support Executive */}
                  <a 
                    href="tel:+919771587519"
                    onClick={(e) => handleDirectAction('support_call', e)}
                    className="flex items-center justify-center gap-2 bg-[#8A2BE2] hover:bg-[#7222cf] text-white px-4 py-3.5 rounded-xl font-sans text-xs font-bold hover:scale-[1.02] active:scale-[0.98] cursor-pointer border border-[#a25cf6] transition-all duration-200 text-center shadow-[0_4px_15px_rgba(138,43,226,0.3)]"
                  >
                    <Headphones className="w-4.5 h-4.5 text-white" />
                    <span>Call Support</span>
                  </a>
                </div>
              </div>
            </form>
          </div>
        </div>

      </div>

      {/* 3. TRANSMISSION SYSTEM LOG UPLINK */}
      {historyLogs.length > 0 && (
        <section className="mt-16 w-full text-left glass-panel p-6 rounded-2xl border-white/5 shadow-lg relative overflow-hidden" id="system-uplink-logs">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-2.5">
              <History className="w-5 h-5 text-tertiary" />
              <h3 className="font-display text-lg font-bold text-white tracking-wide">
                Local Terminal Uplink Logs (Offline Persistence)
              </h3>
            </div>
            
            <button 
              onClick={clearHistory}
              className="flex items-center gap-1.5 text-xs font-sans text-red-400 hover:text-red-300 hover:bg-white/5 px-3 py-1.5 rounded bg-transparent transition-all border border-transparent hover:border-red-500/20 cursor-pointer"
            >
              <Trash2 className="w-3.5 h-3.5" />
              <span>Clear History</span>
            </button>
          </div>

          <div className="flex flex-col gap-3 font-mono text-xs max-h-64 overflow-y-auto pr-2">
            {historyLogs.map((log) => (
              <div 
                key={log.id} 
                className="p-3.5 rounded-lg bg-black/60 border border-outline-variant/20 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 hover:bg-black/90 transition-all"
              >
                <div className="flex flex-col gap-1 text-left">
                  <div className="flex items-center gap-3">
                    <span className="text-[#8A2BE2] font-semibold">{log.id}</span>
                    <span className="text-zinc-500">[{log.timestamp}]</span>
                    <span className="text-zinc-300">Name: <strong className="text-white">{log.name}</strong></span>
                  </div>
                  <div className="text-zinc-400">
                    Type: <span className="text-tertiary font-semibold">{getClassificationLabel(log.classification)}</span>
                  </div>
                  <div className="text-zinc-500 line-clamp-1 italic">
                    Specs: "{log.message}"
                  </div>
                </div>

                <div className="flex items-center gap-2.5 self-start sm:self-center">
                  <div className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse" />
                  <span className="text-green-400 font-semibold text-[10px] tracking-wider uppercase">
                    {log.status}
                  </span>
                </div>
              </div>
            ))}
          </div>
        </section>
      )}

      {/* Spacing gap */}
      <div className="h-12 pointer-events-none" />
    </div>
  );
}
